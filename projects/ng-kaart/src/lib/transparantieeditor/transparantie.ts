...
export interface Opaciteit extends Newtype<{ readonly TRANSPARANTIE: unique symbol }, number> {}

const isoTransparantie: Iso<Transparantie, number> = iso<Transparantie>();
const prismTransparantie: Prism<number, Transparantie> = prism<Transparantie>(n => n >= 0 && n <= 1);

const isoOpaciteit: Iso<Opaciteit, number> = iso<Opaciteit>();
const prismOpaciteit: Prism<number, Opaciteit> = prism<Opaciteit>(n => n >= 0 && n <= 1);

const clampToUnit: Endomorphism<number> = value => Math.max(Math.min(value, 1), 0);
export namespace Transparantie {
  export const opaak: Transparantie = isoTransparantie.wrap(0);

  export const toOpaciteit: Function1<Transparantie, Opaciteit> = transparantie => isoOpaciteit.wrap(1 - toNumber(transparantie));
...
export namespace Opaciteit {
  ...
  export const toTransparantie: Function1<Opaciteit, Transparantie> = opaciteit => isoTransparantie.wrap(1 - toNumber(opaciteit));
  export const fromNumberClamped: Function1<number, Opaciteit> = pipe(
    clampToUnit,
    isoOpaciteit.wrap
  );

  export const fromNumber: PartialFunction1<number, Opaciteit> = prismOpaciteit.getOption;
