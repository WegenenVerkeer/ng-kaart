...
export const literalValueStringRenderer = (literalValue: fed.LiteralValue): string =>
  fltr.matchTypeTypeWithFallback({
    date: () => formateerDateAsDefaultDate(literalValue.value as DateTime),
    range: () => formatRelativeDateRange(literalValue.value as fltr.RelativeDateRange),
