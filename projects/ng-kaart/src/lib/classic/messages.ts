...
export function FeatureGedeselecteerdMsg(
  feature: ol.Feature
): FeatureGedeselecteerdMsg {
...
export function FeatureSelectieAangepastMsg(
  geselecteerdeFeatures: prt.GeselecteerdeFeatures
): FeatureSelectieAangepastMsg {
  return {
...
export function FeatureHoverAangepastMsg(
  hoverFeature: HoverFeature
): FeatureHoverAangepastMsg {
  return { type: "FeatureHoverAangepast", feature: hoverFeature };
}

export function ZichtbareFeaturesAangepastMsg(
  features: Array<ol.Feature>
): ZichtbareFeaturesAangepastMsg {
  return { type: "ZichtbareFeaturesAangepast", features: features };
}

export function TekenGeomAangepastMsg(
  geom: ol.geom.Geometry
): TekenGeomAangepastMsg {
  return { type: "TekenGeomAangepast", geom: geom };
}

export function PrecacheProgressMsg(
  progress: PrecacheLaagProgress
): PrecacheProgressMsg {
  return { type: "PrecacheProgress", progress: progress };
}

export function MijnLocatieStateChangeMsg(
  oudeState: loc.State,
  nieuweState: loc.State,
  event: loc.Event
): MijnLocatieStateChangeMsg {
  return {
    type: "MijnLocatieStateChangeMsg",
    oudeState: oudeState,
    nieuweState: nieuweState,
    event: event,
  };
}

export function LaatsteCacheRefreshMsg(
  laatsteCacheRefresh: LaatsteCacheRefresh
): LaatsteCacheRefreshMsg {
  return {
    type: "LaatsteCacheRefresh",
    laatsteCacheRefresh: laatsteCacheRefresh,
  };
}

export function AchtergrondLagenInGroepAangepastMsg(
  lagen: Array<ToegevoegdeLaag>
): AchtergrondLagenInGroepAangepastMsg {
  return { type: "AchtergrondLagenInGroepAangepast", lagen: lagen };
}

export function VoorgrondLaagLagenInGroepAangepastMsg(
  lagen: Array<ToegevoegdeLaag>
): VoorgrondLaagLagenInGroepAangepastMsg {
  return { type: "VoorgrondLaagLagenInGroepAangepast", lagen: lagen };
}

export function VoorgrondHoogLagenInGroepAangepastMsg(
  lagen: Array<ToegevoegdeLaag>
): VoorgrondHoogLagenInGroepAangepastMsg {
  return { type: "VoorgrondHoogLagenInGroepAangepast", lagen: lagen };
}

export function SubscribedMsg(
  subscription: prt.KaartCmdValidation<prt.SubscriptionResult>,
  reference: any
): SubscribedMsg {
  return {
...
export const ZoomAangepastMsg: (_: number) => ZoomAangepastMsg = (zoom) => ({
  type: "ZoomAangepast",
  zoom: zoom,
});

export const ViewAangepastMsg: (_: prt.Viewinstellingen) => ViewAangepastMsg = (
  vw
) => ({ type: "ViewAangepast", view: vw });

export const MiddelpuntAangepastMsg: (
  _: ol.Coordinate
) => MiddelpuntAangepastMsg = (middelpunt) => ({
...
export const ExtentAangepastMsg: (_: ol.Extent) => ExtentAangepastMsg = (
  ext
) => ({ type: "ExtentAangepast", extent: ext });

export const VectorLagenAangepastMsg: (
  _: Array<ke.ToegevoegdeVectorLaag>
) => VectorLagenAangepastMsg = (lgn) => ({
...
export const lagen: (
  _: VectorLagenAangepastMsg
) => Array<ke.ToegevoegdeVectorLaag> = (msg) => msg.lagen;
export const extent: (_: ExtentAangepastMsg) => ol.Extent = (msg) => msg.extent;
export const view: (_: ViewAangepastMsg) => prt.Viewinstellingen = (msg) =>
  msg.view;
