...
import {
  GoogleWdbLocatieZoekerConfigData,
  ZoekerConfigGoogleWdbConfig,
} from "../config/zoeker-config-google-wdb.config";
import {
  geoJSONOptions,
  IconDescription,
  nietOndersteund,
...
interface ExtendedGeocoderResult
  extends google.maps.GeocoderResult,
    LocatieZoekerLocatie {}
...
export class ZoekerGoogleWdbService implements Zoeker {
  ...
  constructor(
    ...
    this.googleWdbLocatieZoekerConfig = new ZoekerConfigGoogleWdbConfig(
      zoekerConfigData.googleWdb
    );
    ...
    this.googleServicesObs = Observable.create(
      (observer: Observer<GoogleServices>) => {
  ...
  setGoogleWdbLocatieZoekerConfigData(
    googleWdbLocatieZoekerConfigData: GoogleWdbLocatieZoekerConfigData
  ) {
    this.googleWdbLocatieZoekerConfig = new ZoekerConfigGoogleWdbConfig(
      googleWdbLocatieZoekerConfigData
    );
  ...
  zoekresultaten$(opdracht: Zoekopdracht): rx.Observable<ZoekAntwoord> {
    switch (opdracht.zoektype) {
      case "Volledig":
        return this.zoek$(
          opdracht.zoekpatroon,
          "Volledig",
          this.googleWdbLocatieZoekerConfig.maxAantal
        );
      default:
  ...
  help(helpBoom: ZoekerHelpBoom) {
    ...
    if (!this.googleWdbLocatieZoekerConfig.gebruiktPubliekeLocatieZoeker) {
      helpBoom.voegItemToe(
        help.identPlusRefpunt,
        "een locatie",
        "een weglocatie (ident8)",
        "Ident2 of ident8, referentiepunt en afstand"
      );
      helpBoom.voegItemToe(
        help.identPlusHuisnummer,
        "een locatie",
        "een weglocatie (ident8)",
        "Ident2 of ident8, huisnummer en gemeente"
      );
      helpBoom.voegItemToe(
        help.eNummer,
        "een locatie",
        "een weglocatie (ident8)",
        "Europese wegnummer, referentiepunt en afstand"
      );
    } else {
      // enkel de publieke locatiezoeker, enkel volledige wegnummers (europese en ident8en)
      helpBoom.voegItemToe(
        help.wegNummer,
        "een locatie",
        "een weg",
    ...
    switch (zoekterm.type) {
      case "string":
        if (!zoekterm.value || zoekterm.value.trim().length < 3) {
          ...
          return rx.of(
            new ZoekAntwoord(this.naam(), "Volledig", [], [], this.legende)
          );
        }
        ...
        return this.httpClient
          .post<LocatieZoekerSearchResults>(
            this.locatieZoekerUrl + "/zoek",
            body.toString(),
            options
          )
          .pipe(
            switchMap((resp) =>
              this.parseResult(resp, zoektype, maxResultaten)
            ),
  ...
  private vervolledigResultaat(
  ...
  ): Observable<Array<ExtendedResult>> {
    ...
    if (onvolledigeLocatie.alleenGeocoden === true) {
      ...
      const placesSearchObs: Observable<
        ExtendedPlaceResult[]
      > = this.getPlacesTextSearch(omschrijving);

      const predictionsObs: Observable<
        ExtendedGeocoderResult[]
      > = this.getAutocompleteQueryPredictions(omschrijving).pipe(
        concatMap((predictions) => {
          return rx.of(...predictions).pipe(
            take(10), // max 10 predictions
            mergeMap((prediction) => {
              return this.geocodePlace(
                prediction.place_id,
      ...
      const placesAndPredictionsObs = combineLatest(
        placesSearchObs,
        predictionsObs
      ).pipe(
        switchMap(([places, predictions]) => {
          ...
          const ongeveerZelfdeLocatie: (
            bestaande: LatLng,
            nieuw: LatLng
          ) => boolean = (bestaande, nieuw) => {
            return (
              Math.abs(bestaande.lng() - nieuw.lng()) < 0.001 &&
              Math.abs(bestaande.lat() - nieuw.lat()) < 0.001
            );
          };
          ...
          const voegToe = (nieuw) => {
            const zitErAlIn =
              alleResultaten.find((bestaande) =>
                ongeveerZelfdeLocatie(
                  bestaande.geometry.location,
                  nieuw.geometry.location
                )
              ) !== undefined;
            if (!zitErAlIn) {
          ...
          return rx
            .concat(besteResultatenMetGeometrieObs, rx.of(establishments))
            .pipe(
  ...
  private parseResult(
    ...
    const zoekResultaten = new ZoekAntwoord(
      this.naam(),
      zoektype,
      [],
      [],
    ...
    resultaten.errors.forEach((error) =>
      zoekResultaten.fouten.push("Fout: " + error)
    );

    // indien geen locaties gevonden, toon melding
    if (
      resultaten.locaties.length === 0 &&
      resultaten.onvolledigeLocaties.length === 0
    ) {
      ...
      return rx.of(...resultaten.onvolledigeLocaties).pipe(
        ...
        map((resultaten) => {
          return resultaten.map((resultaat) => {
            resultaat.locatie =
              resultaat.locatie ||
              this.wgs84ToLambert72GeoJson(
                resultaat.geometry.location.lng(),
                resultaat.geometry.location.lat()
              );
            return resultaat;
          });
        }),
        map((vervolledigdeLocaties) => {
          ...
          locaties.forEach((locatie, index) => {
            const zoekerType: ZoekerRepresentatieType =
              locatie.bron === "Perceel"
                ? "Perceel"
                : isWdbBron(locatie.bron)
                ? "WDB"
                : "Google";
  ...
  handleError(
    ...
    switch (response.status) {
      ...
      default:
        ...
        error = `Fout bij opvragen locatie: ${
          response.responseText || response.statusText || response
        }`;
    }
    return rx.of(
      new ZoekAntwoord(this.naam(), zoektype, [error], [], this.legende)
    );
  }
    ...
    return this.googleServicesObs.pipe(
      mergeMap((googleServices) => {
        return Observable.create(
          (observer: Observer<ExtendedGeocoderResult[]>) => {
  ...
  private getAutocompleteQueryPredictions(
    omschrijving: string
    ...
    return this.googleServicesObs.pipe(
      mergeMap((googleServices) => {
        return Observable.create(
          (
            observer: Observer<google.maps.places.QueryAutocompletePrediction[]>
          ) => {
              ...
              (predictions, status) => {
                if (status === google.maps.places.PlacesServiceStatus.OK) {
                  observer.next(
                    predictions.filter(
                      (prediction) =>
                        prediction.description.indexOf("in de buurt van") === -1
                    )
                  );
                  observer.complete();
                } else {
      ...
      retryWhen<google.maps.places.QueryAutocompletePrediction[]>(
        this.googleServicesRetryStrategy
      )
    );
  }

  private getPlacesTextSearch(
    omschrijving: string
  ): Observable<ExtendedPlaceResult[]> {
    return this.googleServicesObs.pipe(
      mergeMap((googleServices) => {
        return Observable.create(
          (observer: Observer<ExtendedPlaceResult[]>) => {
                ...
                if (status === google.maps.places.PlacesServiceStatus.OK) {
                  observer.next(
                    results
                      ...
                      .map((result) => {
                        if (
                          result.formatted_address.indexOf(result.name) > -1
                        ) {
                          result.omschrijving = result.formatted_address;
                        } else {
                      ...
                      .filter((result) =>
                        googleServices.boundsVlaanderen.contains(
  ...
  private geocodePlace(
    placeId: string,
    omschrijving: string
  ): Observable<ExtendedGeocoderResult[]> {
    if (placeId) {
      return this.googleServicesObs.pipe(
        mergeMap((googleServices) => {
          return Observable.create(
  ...
  private loadGemeenteGeometrie(
    resultaat: ExtendedResult
  ): Observable<ExtendedResult> {
  ...
  private wgs84ToLambert72GeoJson(lon, lat) {
    const mercatorlonlat = ol.proj.transform(
      [lon, lat],
      "EPSG:4326",
      "EPSG:31370"
    );
  ...
  private loadScript() {
    ...
    node.src = `https://maps.googleapis.com/maps/api/js?key=${this.googleWdbLocatieZoekerConfig.apiKey}&libraries=places&language=nl&callback=__onGoogleLoaded`;
    node.type = "text/javascript";
    document.getElementsByTagName("head")[0].appendChild(node);
  }

  private specificErrorRetryStrategy(
    opts: SpecificErrorRetryStrategyOpts
    ...
    return (attempts: Observable<any>) => {
      return attempts.pipe(
        mergeMap((error, i) => {
          ...
          if (
            retryAttempt > opts.maxRetryAttempts ||
            !opts.errorMessagesForRetry.find((e) => e === error)
          ) {
            kaartLogger.warn(
              `Retry: error na ${retryAttempt} pogingen: ${error}`
            );
            return rx.throwError(error);
          }
          kaartLogger.info(
            `Retry: poging ${retryAttempt}: probeer opnieuw in ${opts.retryDelay}ms`
          );
