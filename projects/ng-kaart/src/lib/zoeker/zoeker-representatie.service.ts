...
export const ZOEKER_REPRESENTATIE = new InjectionToken<
...
@Injectable()
export class DefaultRepresentatieService
  implements AbstractRepresentatieService {
  ...
  constructor(
    ...
    function vervangKleur(pin: string, kleur: ol.Color): string {
      function vullFillIn(
        nodes: HTMLCollectionOf<SVGGraphicsElement>,
        fillKleur: string
      ) {
    ...
    this.locatieServicesConfig = new ZoekerConfigLocatorServicesConfig(
      zoekerConfigData.locatorServices
    );
    this.googleLocatieZoekerConfig = new ZoekerConfigGoogleWdbConfig(
      zoekerConfigData.googleWdb
    );

    this.matIconRegistry.addSvgIcon(
      crabSvgNaam,
      this.sanitizer.bypassSecurityTrustResourceUrl(encodeAsSvgUrl(crabMarker))
    );
    this.matIconRegistry.addSvgIcon(
      googleSvgNaam,
      this.sanitizer.bypassSecurityTrustResourceUrl(
        encodeAsSvgUrl(googleMarker)
      )
    );
    this.matIconRegistry.addSvgIcon(
      wdbSvgNaam,
      this.sanitizer.bypassSecurityTrustResourceUrl(encodeAsSvgUrl(wdbMarker))
    );
    this.matIconRegistry.addSvgIcon(
      perceelSvgNaam,
      this.sanitizer.bypassSecurityTrustResourceUrl(
        encodeAsSvgUrl(perceelMarker)
      )
    );

    this.googleStyle = maakStyle(
      this.googleLocatieZoekerConfig.kleur,
      googleMarker
    );
    this.googleHighlightStyle = maakStyle(
      this.googleLocatieZoekerConfig.kleur,
      googleHighlightMarker
    );
    this.wdbStyle = maakStyle(this.googleLocatieZoekerConfig.kleur, wdbMarker);
    this.wdbHighlightStyle = maakStyle(
      this.googleLocatieZoekerConfig.kleur,
      wdbHighlightMarker
    );
    this.crabStyle = maakStyle(this.locatieServicesConfig.kleur, crabMarker);
    this.crabHighlightStyle = maakStyle(
      this.locatieServicesConfig.kleur,
      crabHighlightMarker
    );
    this.perceelStyle = maakStyle(
      this.locatieServicesConfig.kleur,
      perceelMarker
    );
    this.perceelHighlightStyle = maakStyle(
      this.locatieServicesConfig.kleur,
      perceelHighlightMarker
    );
  }
