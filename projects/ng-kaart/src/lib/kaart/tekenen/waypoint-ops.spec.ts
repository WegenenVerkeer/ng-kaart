...
import { addWaypoint, emptyRouteState, removeWaypoint, RouteState } from "./waypoint-ops";
...
describe("Route Changes bij verwijderen:", () => {
  ...
  describe("Wanneer een midden waypoint verwijderd wordt", () => {
    it("moet er 2 routes verdwijnen en 1 in de plaats komen", () => {
      ...
      const removed1 = array.findFirst(transition4.snd.routesRemoved, pr => pr.begin === waypoint0).toNullable();
      expect(removed1.end).toBe(waypoint1);
      const removed2 = array.findFirst(transition4.snd.routesRemoved, pr => pr.end === waypoint2).toNullable();
...
describe("Route Changes bij toevoegen in het midden:", () => {
  describe("Wanneer er een waypoint in het midden toegevoegd wordt", () => {
    it("moeten er een 2 nieuwe routes zijn, en 1 route moet verdwijnen", () => {
      ...
      const added1 = array.findFirst(transition3.snd.routesAdded, pr => pr.end === waypoint2).toNullable();
      expect(added1.begin).toBe(waypoint0);
      const added2 = array.findFirst(transition3.snd.routesAdded, pr => pr.begin === waypoint2).toNullable();
