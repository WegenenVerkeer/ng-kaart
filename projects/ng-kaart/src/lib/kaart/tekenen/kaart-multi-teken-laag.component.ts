...
import { Endomorphism, Function1, Function2, Function3, identity, Lazy, pipe, Refinement } from "fp-ts/lib/function";
...
function drawStateTransformer(
  ...
  const sendFullGeometry: Consumer<ol.Feature[]> = features => {
    const maybeLine = array
      ...
      .map(coordinates => new ol.geom.LineString(coordinates));
  ...
  switch (ops.type) {
    ...
    case "DeletePoint": {
      ...
      dispatchCmd(prt.VervangFeaturesCmd(PuntLaagNaam, List(newFeatures), kaartLogOnlyWrapper));
      sendFullGeometry(newFeatures);
      return applySequential([pointFeaturesLens.set(newFeatures), firstPointFeatureLens.modify(fp => (isEmpty(newFeatures) ? none : fp))]);
    }
  }
}
