...
export class FeatureTabelSettingsComponent extends KaartChildDirective {
  ...
  constructor(
    ...
    matIconRegistry.addSvgIcon(
      "settings",
      domSanitize.bypassSecurityTrustResourceUrl(encodeAsSvgUrl(settingsSvg))
    );
    ...
    this.runInViewReady(
      rx
        .merge(
          this.actionFor$("makeCompact").pipe(
            mapTo(TableModel.setCompactLayout)
          ),
          this.actionFor$("makeComfortable").pipe(
            mapTo(TableModel.setComfortableLayout)
          )
        )
