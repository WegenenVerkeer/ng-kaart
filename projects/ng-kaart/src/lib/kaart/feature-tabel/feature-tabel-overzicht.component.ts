...
export class FeatureTabelOverzichtComponent extends KaartChildDirective {
  ...
  constructor(kaart: KaartComponent, ngZone: NgZone) {
    ...
    const voorgrondLagen$ = this.modelChanges.lagenOpGroep[
      "Voorgrond.Hoog"
    ].pipe(map(array.filter(ke.isToegevoegdeVectorLaag)), share());

    const updateLagen$ = voorgrondLagen$.pipe(
      map((lagen) =>
        lagen
          .filter((laag) => laag.magGetoondWorden)
          .filter(ke.isToegevoegdeVectorLaag)
      ),
      map(TableModel.updateLagen)
    );

    const updateZoomAndExtent$ = this.modelChanges.viewinstellingen$.pipe(
      map(TableModel.updateZoomAndExtent)
    );

    const zichtbareFeatures$ = this.modelChanges.zichtbareFeaturesPerLaag$.pipe(
      map(TableModel.updateZichtbareFeatures)
    );

    const featureSelection$ = this.modelChanges.geselecteerdeFeatures$.pipe(
      map(TableModel.updateSelectedFeatures)
    );

    const filterGezet$ = this.modelChanges.laagfilterGezet$.pipe(
      map(TableModel.updateFilterSettings)
    );
    ...
    const delayedUpdates$: rx.Observable<TableModel.TableModelUpdate> = asyncUpdatesSubj.pipe(
      map(Update.createSync)
    );

    const clientUpdateSubj: rx.Subject<TableModel.TableModelUpdate> = new rx.Subject();
    this.tableUpdater = (update: TableModel.TableModelUpdate) =>
      clientUpdateSubj.next(update);
    ...
    const externeInstellingUpdate$ = this.modelChanges.tabelLaagInstellingen$.pipe(
      map(TableModel.updateLaagInstellingen)
    );
              ...
              scan((model: TableModel, update: TableModel.TableModelUpdate) => {
                ...
                update
                  ...
                  .subscribe({
                    ...
                    error: (err) =>
                      kaartLogger.error("Probleem bij async model update", err), // Moet ook in UI komen. Evt retry
                  });
                return newModel;
    ...
    this.laagModel$ = (titel) =>
      this.tableModel$.pipe(collectOption(TableModel.laagForTitel(titel)));
