...
export class LaagmanipulatieComponent extends KaartChildDirective {
  ...
  constructor(
    ...
    matIconRegistry.addSvgIcon(
      "spinner",
      domSanitizer.bypassSecurityTrustResourceUrl(encodeAsSvgUrl(spinnerSVG))
    );
    matIconRegistry.addSvgIcon(
      "transparant",
      domSanitizer.bypassSecurityTrustResourceUrl(
        encodeAsSvgUrl(transparancySVG)
      )
    );
    ...
    const zichtbaar$ = this.zoom$.pipe(
      map(
        (zoom) =>
          zoom >= this.laag.bron.minZoom && zoom <= this.laag.bron.maxZoom
      ),
  ...
  pasStijlAan() {
    this.dispatch(
      cmd.BewerkVectorlaagstijlCmd(this.laag as ke.ToegevoegdeVectorLaag)
    );
  }

  pasFilterAan() {
    this.dispatch(
      cmd.BewerkVectorFilterCmd(this.laag as ke.ToegevoegdeVectorLaag)
    );
  }

  verwijderFilter() {
    this.dispatch(
      cmd.ZetFilter(this.laag.titel, Filter.empty(), kaartLogOnlyWrapper)
    );
  }
