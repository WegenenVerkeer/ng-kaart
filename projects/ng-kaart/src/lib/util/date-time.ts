...
export const formateerDate: Curried2<
...
> = (maybeFormat) => (date) => {
  return date
    .setLocale("nl-BE")
    .toFormat(maybeFormat.getOrElse(defaultDateFormat));
};
...
export const formateerDateTime: Curried2<
  option.Option<string>,
  DateTime,
  string
> = (maybeFormat) => (dateTime) => {
  return dateTime
    .setLocale("nl-BE")
    .toFormat(maybeFormat.getOrElse("dd/MM/yyyy hh:mm:ss"));
};

export const parseDate: Curried2<
  option.Option<string>,
  string,
...
const parseDateTimeWithFormat: Function1<
  string,
  PartialFunction1<string, DateTime>
> = (format) => (text) => {
  return option.fromPredicate((d: DateTime) => d.isValid)(
...
export const parseDateTime: Curried2<
  option.Option<string>,
  string,
  option.Option<DateTime>
> = (maybeFormat) => (text) =>
