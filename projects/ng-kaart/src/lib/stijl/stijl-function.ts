...
import {
  ...
  jsonAwvV0RuleInterpreter,
} from "./json-awv-v0-stijlfunctie";
...
export function definitieToStyleFunction(
  encoding: string,
  definitieText: string
): Validation<ol.style.StyleFunction> {
  return chain(
    validateAsDynamicStyle(encoding, definitieText),
    validateAwvV0RuleDefintion
  );
}

export function validateAsDynamicStyle(
  encoding: string,
  definitieText: string
): Validation<AwvV0DynamicStyle> {
...
export const validateAwvV0RuleDefintion: Validator<
  AwvV0DynamicStyle,
