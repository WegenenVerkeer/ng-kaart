      ...
      .pipe(
        ...
        switchMap((coordinaat: ol.Coordinate) =>
          Observable.merge(
            ...
            srv.adresViaXY$(this.http, coordinaat).pipe(map(adres => srv.withAdres(coordinaat, adres)))
          ).pipe(scan((nieuw, bestaand) => this.verrijk(nieuw, bestaand), srv.fromCoordinate(coordinaat)))
        ),
